...
if (!(Test-Python3)) {
    ...
    if (!(Test-Python3)) {
        $python3 = "py"
        if (!(Test-Python3)) {
            "`nERROR: Python 3 must be runnable via python3.exe, python.exe, or py"
            exit
        }
    }
}
...
& $python3 aws-iot-device-sdk-python-v2\samples\pubsub.py --endpoint auvjp65wb96hw-ats.iot.us-west-2.amazonaws.com --ca_file root-CA.crt --cert TrashBin_1.cert.pem --key TrashBin_1.private.key --client_id basicPubSub --topic sdk/test/python --count 0